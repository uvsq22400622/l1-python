...
def applique_flou_dynamique(flou_facteur):
    global matrice_pixels, matrice_pixels_affichee
    if matrice_pixels is not None :
        flou_facteur = float(flou_facteur)
        noyau = noyau_facteur(flou_facteur)
        for i in range(3):
            canal = matrice_pixels[:,:,i]
            matrice_pixels_flou = convolve2d(canal,noyau, mode="same", boundary="symm")
            matrice_pixels[:,:,i] = np.clip(matrice_pixels_flou, 0,255)
        matrice_pixels_affichee = matrice_pixels.astype(np.uint8)
        rafraichir(matrice_pixels_affichee)

def noyau_facteur(flou_facteur):
    """Créer un noyau de flou"""
    taille = int(flou_facteur)
    if taille % 2 == 0: #doit être impair
        taille += 1 
    noyau = np.ones((taille,taille)) / (taille*taille)
    return noyau

def ouvrir_dialogue_flou_dynamique():
    global dialogue_effet
    dialogue_effet = tk.Toplevel(fenetre_principale)
    dialogue_effet.title("Flou")
    dialogue_effet.geometry("350x150")
    dialogue_effet.grab_set()

    slider = tk.Scale(dialogue_effet, from_=1, to=25, orient=tk.HORIZONTAL, 
    length=200, resolution=1, digits=2,
    command=applique_flou_dynamique)

    slider.set(3)  #valeur initiale
    slider.pack(pady=20)
...
menu_effets.add_command(
    ...
    command=ouvrir_dialogue_flou_dynamique
)
